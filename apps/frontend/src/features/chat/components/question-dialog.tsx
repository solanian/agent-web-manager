...
export function QuestionDialog({
	...
	const savedSelectionsRef = useRef<
		Map<
			number,
			{ selectedIndex: number; multiSelected: Set<number>; otherText: string }
		>
	>(new Map());
	...
	const focusOtherInputAfterToggle = useCallback(
				...
				if (prevAnswer) {
					if (targetQuestion.multi_select) {
						...
						const knownLabels = new Set(
							targetQuestion.options.map((o: any) => o.label),
						);
						const selected = new Set<number>();
						targetQuestion.options.forEach((o: any, i: number) => {
				...
				<div className="flex flex-col px-4 py-2 gap-0.5">
					...
					<div
						className={cn(
							...
							isMultiSelect &&
								multiSelected.has(otherIndex) &&
								"bg-primary/[0.08]",
							selectedIndex === otherIndex &&
								!(isMultiSelect && multiSelected.has(otherIndex)) &&
								"bg-muted/70",
							selectedIndex === otherIndex &&
								isMultiSelect &&
								multiSelected.has(otherIndex) &&
					...
					>
						...
						<div className="flex flex-col min-w-0 flex-1">
							...
							<input
								...
								placeholder={
									currentQuestion.other_label || "Type your answer..."
								}
								className={cn(
