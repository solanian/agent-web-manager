...
export const PromptToolbar = memo(function PromptToolbarComponent({
	...
	const hasChanges = Boolean(
		stats?.isGitRepo && stats.hasChanges && stats.files && !stats.error,
	);
	...
	return (
		<div
			className={cn(
				"w-full px-1 sm:px-2 flex flex-col gap-1 mb-2",
				isGitDiffLoading && "opacity-70",
			)}
		>
			{/* ── Expanded panel ── */}
			{activeTab && (
				...
				>
					...
					{activeTab === "todo" && <ToolbarTodoPanel items={todoItems} />}
				</div>
			)}
