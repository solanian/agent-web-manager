...
function VirtuosoScrollerComponent(
	...
	return (
		<div
			ref={ref}
			className={cn(
				"flex-1 overflow-y-auto overflow-x-hidden pr-0 sm:pr-2",
				className,
			)}
			{...rest}
		/>
...
function VirtuosoListComponent(
	...
	return (
		<div
			ref={ref}
			className={cn("flex flex-col px-0.5 py-4 sm:px-6 lg:px-8", className)}
			{...rest}
		/>
	);
}
                  ...
                  hover-reveal
                   opacity-0 group-hover:opacity-100 transition-opacity mt-1"
										>
											...
											{onForkSession && message.turnIndex !== undefined && (
												<MessageForkButton
													onFork={() => onForkSession(message.turnIndex!)}
												/>
											)}
										</MessageActions>
									)}
