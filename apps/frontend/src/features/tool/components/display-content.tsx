...
const DiffContent = ({ data }: { data: DiffDisplayData }) => {
	...
	const { addedLines, removedLines, totalLines, truncatedHunks } =
		useMemo(() => {
			if (!file) {
				return {
					addedLines: 0,
...
const ShellContent = ({ data }: { data: ShellDisplayData }) => (
	<CodeBlock code={data.command} language={data.language || "bash"} />
);
...
const DisplayItemRenderer = ({ item }: { item: DisplayItem }) => {
	switch (item.type) {
		...
		case "brief": {
			...
			const briefItem = item as unknown as {
				type: string;
				text?: string;
				data?: unknown;
			};
			const briefText = briefItem.text ?? String(briefItem.data ?? "");
			return <pre className="whitespace-pre-wrap text-xs">{briefText}</pre>;
		}
